...
public class GunScript : MonoBehaviour {
	...
	void Awake(){
        ...
        ReloadButton = GameObject.Find("Reload");
        AimButton = GameObject.Find("Aim");
        Color = GameObject.Find("Color");
        //button.GetComponent<Fireing>().WesponOn = true;
        // GameObject.ToString()
        //button.GetComponent<Fireing>().NameOfWespon = "NewGun_semi(Clone)";
	...
	*/
	 ...
	 */
	void Shooting(){
        if (FireButton.GetComponent<Fireing>().buttonPressed)
        {
            Fire = true;
        }
	...
	public void ShootMethod(){
		...
		if(waitTillNextFire <= 0 && !reloading && pmS.maxSpeed < 5){
			...
			if(bulletsInTheGun > 0){
                Color.GetComponent<CheckingHit>().Fire = true;
	...
	*/
	void Animations(){
		...
		if(handsAnimator){
            ...
            if (ReloadButton.GetComponent<Fireing>().buttonPressed)
            {
                Reload = true;
            }
            if ((Input.GetKeyDown(KeyCode.R) && pmS.maxSpeed < 5 && !reloading && !meeleAttack/* && !aiming*/) || Reload){
				StartCoroutine("Reload_Animation");
                Reload = false;
			}
		}
